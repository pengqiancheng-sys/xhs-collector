...
(() => {
  ...
  function bindElements() {
    const ids = [
      'cfg-appId','cfg-appSecret','cfg-appToken','cfg-tableId','cfg-tableName',
    ...
    DOM['btn-fetch-fields'].addEventListener('click', async () => {
      ...
      try {
        const r = await send({
          ...
          appToken: DOM['cfg-appToken'].value,
          tableId: DOM['cfg-tableId'].value,
          appId: DOM['cfg-appId'].value,
          appSecret: DOM['cfg-appSecret'].value,
        });
        if (r.success) {
